...
var connectCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if socketType == "database" {
			if upstreamType != "tls" && upstreamType != "mysql" && upstreamType != "postgres" && upstreamType != "" {
				log.Fatalf("error: --upstream_type should be tls for mysql tls or mysql or postgres, defaults to mysql")
			}
		}
...
func init() {
	...
	connectCmd.Flags().StringVarP(&upstream_type, "upstream_type", "", "", "Upstream type: Upstream type: http, https for http socket or mysql, tls (mysql), postgres for database sockets")
